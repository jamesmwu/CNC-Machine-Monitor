    ...
    public class MTConnectClient : IMTConnectClient, IDisposable
    {
        ...
        private void ParseStream(IRestResponse response)
        {
            ...
            using (StringReader sr = new StringReader(response.Content))
            {
                ...
                //getting exception because in to Attribute is NULL
                var element = xdoc.Descendants().First(e => e.Name.LocalName == "Header").Attribute("lastSequence");
                if (element != null) _lastSequence = Convert.ToInt64(element.Value);
                else _lastSequence = 0;

                var xmlDataItems = xdoc.Descendants().Where(e => e.Attributes().Any(a => a.Name.LocalName == "dataItemId"));
                ...
                if (xmlDataItems.Any())
                {
                    ...
                    foreach (var item in dataItems)
                    {
                        ...
                        DataItemChangedHandler(dataItem);
                    }
                }
            }
        }
